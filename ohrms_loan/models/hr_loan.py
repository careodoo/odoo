...
class HrLoan(models.Model):
    ...
    def compute_installment(self):
        ...
        company based on payment start date and the no of installments.
            """
        for loan in self:
            if loan.state == 'approve' or any(line.paid for line in loan.loan_lines):
                raise UserError(_(
                    "You cannot recompute installments for an approved loan or a "
                    "loan that already has paid installments, as this would discard "
                    "the existing schedule. Cancel the loan first if you need to "
                    "rebuild it."))
