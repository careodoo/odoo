...
class ProposalEquipmentReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, objs):
        ...
        # equipment
        row = 5
        sheet.merge_range(row, 0, row, 3, 'Equipments', header_style3)
        row += 1
        ...
        row += 1
        for line in proposal.equipment_ids:
            sheet.write(row, 0, line.product_id.name, header_style)
            sheet.write(row, 1, line.quantity, header_style1)
            sheet.write(row, 2, line.cost, header_style1)
            sheet.write(row, 3, line.total_amount, header_style1)
            row += 1
