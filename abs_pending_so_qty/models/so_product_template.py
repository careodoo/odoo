...
class SaleOrderProductTemplate(models.Model):
    ...
    def _compute_pending_sale_order_qty(self):
        for product in self:
            ...
            if product:
                sale_order_qty = 0
                sale_order_line = self.env['sale.order.line'].search([('product_id.product_tmpl_id','=',product.id),('order_id.state','not in',('draft','sale','cancel'))])
                if sale_order_line:
    ...
    def display_pending_sale_order_qty(self):        
        if self.id:
            ...
            product_ids=[]
            so_line = self.env['sale.order.line'].search([('product_id.product_tmpl_id','=',self.id),('order_id.state','not in',('draft','sale','cancel'))])
