odoo.define('care_approvals.form_widgets', function (require) {
	"use strict";
    ...
    var One2manyApproval = FieldOne2Many.extend({
		template: 'One2manyApproval',
		events: {
			"click #button-create-sale-order": "create_sale_order",
			"click #button-create-purchase-order": "create_purchase_order",
		...
		create_sale_order: function()
		{
			...
			var w_response = confirm("Are You Sure ?");
			if (w_response) {
                self.do_action({
                    ...
                    target: 'new',
                    context: {
                        'default_product_ids': selected_lines,
                        'default_type': 'so',
                    },
                });
             }
		},
		create_purchase_order: function()
		{
			var self=this;
			var current_model = this.recordData[this.name].model;
			var selected_lines = self.find_deleted_lines();
	...
	fieldRegistry.add('one2many_approval', One2manyApproval);
});