import 'package:flutter/material.dart';
import 'package:provider/provider.dart';
import '../../core/auth.dart';
import '../../core/i18n.dart';
import 'c2c_shell.dart';
import 'c2c_contracts.dart';

class C2CAccountScreen extends StatefulWidget {
  const C2CAccountScreen({super.key, this.canSwitchCafm = false});
  final bool canSwitchCafm;
  @override
  State<C2CAccountScreen> createState() => _C2CAccountScreenState();
}

class _C2CAccountScreenState extends State<C2CAccountScreen> {
  Future<Map<String, dynamic>>? _acc;

  @override
  void initState() {
    super.initState();
    _acc = context.read<AuthProvider>().api.c2cAccount();
  }

  @override
  Widget build(BuildContext context) {
    final name = context.read<AuthProvider>().profile?.name ?? '';
    return Scaffold(
      backgroundColor: C2C.bg,
      body: FutureBuilder<Map<String, dynamic>>(
        future: _acc,
        builder: (_, snap) {
          final d = snap.data ?? {};
          return ListView(children: [
            Container(
              decoration: const BoxDecoration(gradient: LinearGradient(colors: [C2C.navy, C2C.navy2]), borderRadius: BorderRadius.vertical(bottom: Radius.circular(24))),
              padding: const EdgeInsets.fromLTRB(18, 0, 18, 22),
              child: SafeArea(
                bottom: false,
                child: Column(children: [
                  const SizedBox(height: 8),
                  CircleAvatar(radius: 34, backgroundColor: Colors.white, child: Text(name.isNotEmpty ? name.trim().characters.first : '?', style: const TextStyle(color: C2C.navy, fontSize: 28, fontWeight: FontWeight.w900))),
                  const SizedBox(height: 10),
                  Text(name, style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w900)),
                  if (d['phone'] != null) Text('${d['phone']}', style: TextStyle(color: Colors.white.withValues(alpha: 0.85), fontSize: 13)),
                ]),
              ),
            ),
            Padding(
              padding: const EdgeInsets.all(16),
              child: Row(children: [
                _stat('${d['total_bookings'] ?? 0}', tr('إجمالي الحجوزات', 'Bookings'), Icons.event_note_rounded, C2C.navy),
                const SizedBox(width: 10),
                _stat('${d['upcoming'] ?? 0}', tr('قادمة', 'Upcoming'), Icons.upcoming_rounded, const Color(0xFFF59E0B)),
                const SizedBox(width: 10),
                _stat('${d['total_spent'] ?? 0}', tr('أنفقت', 'Spent'), Icons.payments_rounded, const Color(0xFF16A34A)),
              ]),
            ),
            _tile(Icons.event_note_rounded, tr('حجوزاتي', 'My bookings'), () {}),
            _tile(Icons.description_outlined, tr('طلبات التعاقد وعروض الأسعار', 'Contracts & quotes'), () => Navigator.push(context, MaterialPageRoute(builder: (_) => const C2CContractsScreen()))),
            if (widget.canSwitchCafm)
              _tile(Icons.apartment_rounded, tr('التحويل إلى إدارة المرافق (CAFM)', 'Switch to CAFM'), () => openCafm(context), color: C2C.navy),
            _tile(Icons.language_rounded, tr('اللغة', 'Language'), _langSheet),
            _tile(Icons.logout_rounded, tr('تسجيل الخروج', 'Sign out'), () => context.read<AuthProvider>().logout(), color: C2C.red),
            const SizedBox(height: 20),
            Center(child: Text('CARE 2 CARE', style: TextStyle(color: Colors.grey.shade400, fontWeight: FontWeight.w900, letterSpacing: 1))),
            const SizedBox(height: 30),
          ]);
        },
      ),
    );
  }

  Widget _stat(String v, String l, IconData ic, Color c) => Expanded(
        child: Container(
          padding: const EdgeInsets.symmetric(vertical: 14, horizontal: 8),
          decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), boxShadow: const [BoxShadow(color: Colors.black12, blurRadius: 5, offset: Offset(0, 2))]),
          child: Column(children: [
            Icon(ic, color: c, size: 22),
            const SizedBox(height: 6),
            Text(v, style: TextStyle(fontWeight: FontWeight.w900, fontSize: 18, color: c)),
            Text(l, style: const TextStyle(fontSize: 10.5, color: Colors.grey), textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis),
          ]),
        ),
      );

  Widget _tile(IconData ic, String t, VoidCallback onTap, {Color color = C2C.navy}) => Padding(
        padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 4),
        child: Material(
          color: Colors.white, borderRadius: BorderRadius.circular(14),
          child: ListTile(
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
            leading: Icon(ic, color: color),
            title: Text(t, style: TextStyle(fontWeight: FontWeight.w700, color: color == C2C.red ? C2C.red : null)),
            trailing: const Icon(Icons.chevron_right_rounded, color: Colors.grey),
            onTap: onTap,
          ),
        ),
      );

  void _langSheet() => showModalBottomSheet(context: context, builder: (_) => SafeArea(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          for (final l in const [['ar', '🇰🇼 العربية'], ['en', '🇬🇧 English']])
            ListTile(title: Text(l[1]), trailing: gLang == l[0] ? const Icon(Icons.check, color: C2C.navy) : null, onTap: () { context.read<LangProvider>().setLang(l[0]); Navigator.pop(context); setState(() {}); }),
        ]),
      ));
}
